...
Each accepted turn also has a Renalo-owned internal turn marker. On the first model step of an established conversation, Renalo offers a strict no-argument `recommend_new_chat` control tool. The prompt allows the model to call it alone, before prose or financial tools, only for a significant unrelated topic change. It is not offered on the conversation's first turn or on later model/tool steps, and no extra classifier request is made.

A recommendation persists as an internal pending event containing the marked turn, control-call ID, original input, and detection-phase metrics. Continuing here appends the matching control-tool output and a decision event, then resumes the same Responses turn. Continuing in a new chat creates the destination conversation and records a redirect decision before submitting the original input through the ordinary new-chat flow. Redirected turn rows remain physically append-only, but event loading omits the entire marked turn from the old conversation's model replay and browser history. Redirect filtering happens before interrupted-call repair so the hidden control call cannot be repaired back into the old topic.
...
A pending topic-change recommendation renders as an application-owned informational alert, not assistant prose. It blocks additional composer input until the user chooses. The primary action transfers the pending user input to a new conversation and removes that turn from the old conversation projection; the secondary action resumes the same turn in place. Both decisions remain user-controlled, and the recommendation survives page reloads without exposing its raw function call.
