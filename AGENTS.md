...
- Income and expense rows share the type-discriminated `Transaction` backend model and transaction-oriented API; current expense pages call transaction endpoints with `TransactionType.EXPENSE`, and income UI/API support is pending unless explicitly requested.
