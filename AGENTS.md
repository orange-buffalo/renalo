...
- Current account balances are calculated as of the current UTC date, inclusive; future transactions, transfers, and adjustments must be excluded consistently. Monetary aggregate and merge arithmetic must use exact `Long` operations and fail on overflow rather than wrap.
