...
- Transaction default-currency values are persisted rebuildable analytics projections under the current default account currency. Same-currency values equal the transaction amount; foreign values use deterministic direct-transfer evidence, preferring the expected income/expense flow and date side, same foreign account, exact foreign amount, nearest date, then transfer ID. Conversion uses the authoritative transfer-side minor-unit ratio with positive half-up rounding and exact overflow checks. Missing evidence stays explicitly `UNAVAILABLE`; do not silently substitute external market rates. Maintain projections with set-based, narrowly scoped recalculation: transaction writes and recurring generation use changed IDs, transfer writes use directly affected foreign currencies, and non-default account currency changes use old/new currencies plus transfer-counterpart currencies when the account enters or leaves the default currency. Serialize projection recalculations with the per-user transaction advisory lock, recalculate full history only when the default currency changes, and batch all ID, currency, full-history, and migration scopes at 500 transactions per update. Existing rows are backfilled once by Flyway; do not add startup history rebuilds or modify an applied migration.
...
- Account adjustments (`AccountAdjustment`) are manual balance corrections linked to a `TrackingAccount` and user. They are stored in a separate `account_adjustments` table and factored into balance through `AccountAdjustmentService`. Adjustments affect `DashboardService` total balance and the adjustments page's current balance. Each adjustment has a date-only `LocalDate` field used for display; UI-created adjustments use the current date in the browser timezone, while Toshl imports preserve the CSV row date.
...
- Standard single-form pages must keep their full cancel/submit action bar fixed to the bottom of the mobile viewport with safe-area padding and reserve enough form space to prevent the bar from covering the final fields. Do not apply this pattern to pages with multiple independent forms whose action bars would overlap.
- Standard text inputs and dropdowns should be capped at 300px wide only within two-column form grids (`.tracking-account-form`); outside form grids there is no default cap. Money inputs are capped at 200px wide within form grids. On mobile, all max-width caps are removed.
